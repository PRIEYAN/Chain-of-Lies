...
export default function SecretWordDisplay() {
  const { role, encryptedWord, secretWord, taskProgress, decryptedPercentage } = useGameStore();
  ...
  const displayWord = isCrew ? (secretWord || encryptedWord) : encryptedWord;

  return (
    <div className="p-3 bg-white/5 rounded-lg border border-white/10">
      <div className="text-xs text-muted-foreground uppercase tracking-widest text-blue-400">{label}</div>
      <div className="font-mono text-2xl tracking-widest mt-2 animate-pulse">{displayWord ?? "----"}</div>
